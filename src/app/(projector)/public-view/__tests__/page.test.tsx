...
describe('PublicInstructorView projector channel', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers();
  });
  ...
  test('subscribes to projector channel in addition to shared channel', async () => {
    mockChannel.mockImplementation(() => ({
      on: jest.fn().mockReturnThis(),
    ...
    // Should subscribe to both shared and projector channels
    expect(mockChannel).toHaveBeenCalledWith('session:test-session-id');
    expect(mockChannel).toHaveBeenCalledWith('session:test-session-id:projector');
  });

  test('displays solution code when solution_displayed event received with show: true', async () => {
    let solutionCallback: ((payload: any) => void) | null = null;

    mockChannel.mockImplementation((channelName?: string) => {
      const mock: any = {
        on: jest.fn((type, options, callback) => {
          if (channelName?.endsWith(':projector') && type === 'broadcast' && options?.event === 'solution_displayed') {
    ...
    mockFetch.mockResolvedValueOnce({
      ...
      json: async () => ({
        sessionId: 'test-session-id',
        joinCode: 'ABC-123',
        problem: { title: 'Test Problem', description: 'desc' },
        featuredStudentId: 'student-1',
        featuredCode: 'print("student code")',
        hasFeaturedSubmission: true,
      }),
    });
    ...
    await waitFor(() => {
      expect(screen.getByTestId('code-content')).toHaveTextContent('print("student code")');
    });

    // Broadcast solution
    await act(async () => {
      solutionCallback!({
        payload: { show: true, solution: 'print("solution code")' },
      });
    });

    // Solution should replace student code
    await waitFor(() => {
      expect(screen.getByTestId('code-content')).toHaveTextContent('print("solution code")');
    });
  });

  test('returns to featured student code when solution_displayed event received with show: false', async () => {
    let solutionCallback: ((payload: any) => void) | null = null;
    ...
    await act(async () => {
      solutionCallback!({
        payload: { show: false, solution: '' },
      });
    });

    // Should return to student code
    await waitFor(() => {
      expect(screen.getByTestId('code-content')).toHaveTextContent('print("student code")');
    });
  });

  test('shows Solution title when displaying solution', async () => {
    let solutionCallback: ((payload: any) => void) | null = null;
    ...
    await act(async () => {
      solutionCallback!({
        payload: { show: true, solution: 'print("solution")' },
      });
    });

    await waitFor(() => {
      expect(screen.getByTestId('code-title')).toHaveTextContent('Solution');
    });
  });

  test('cleans up projector channel on unmount', async () => {
    mockChannel.mockImplementation(() => ({
      on: jest.fn().mockReturnThis(),
      subscribe: jest.fn((callback) => { callback('SUBSCRIBED'); }),
    }));
    ...
    const PublicInstructorView = require('../page').default;
    const { unmount } = render(<PublicInstructorView />);
