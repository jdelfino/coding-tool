...
describe('useRevisionHistory', () => {
  ...
  describe('WebSocket mode', () => {
    it('uses WebSocket when sendMessage is provided', () => {
      ...
      expect(mockSendMessage).toHaveBeenCalledWith('GET_REVISIONS', {
        studentId: 'student-1',
      });
      expect(global.fetch).not.toHaveBeenCalled();
    });

    it('processes WebSocket REVISIONS_DATA message', async () => {
      const mockSendMessage = jest.fn();
      ...
      rerender({ lastMessage: mockLastMessage });

      await waitFor(() => expect(result.current.revisions).toHaveLength(1));

      expect(result.current.revisions[0].code).toBe('print("Hello")');
      expect(result.current.loading).toBe(false);
    });

    it('ignores messages for different session/student', async () => {
      const mockSendMessage = jest.fn();
      const mockLastMessage = {
        type: 'REVISIONS_DATA',
        payload: {
          ...
          revisions: [{ id: 'rev-1', timestamp: '2024-01-01T10:00:00Z', code: 'print("Hello")' }],
        },
      };
